...
class DrafterController extends Controller
{
    ...
    public function datatable(Request $request){
        ...
        $total_data = Drafter::count();
        
        $query_data = Drafter::where(function($query) use ($search, $request) {
                if($search) {
        ...
        $total_filtered = Drafter::where(function($query) use ($search, $request) {
                if($search) {
                    $query->where(function($query) use ($search, $request) {
                        $query->where('code', 'like', "%$search%")
                            ->orWhere('note', 'like', "%$search%")
    ...
    public function create(Request $request){
        ...
        try {
            $validation = Validator::make($request->all(), [
                'code'		            => $request->temp ? ['required', Rule::unique('drafters', 'code')->ignore(CustomHelper::decrypt($request->temp),'code')] : 'required|unique:drafters,code',
                'project_id'            => 'required',
                'post_date'             => 'required',
            ], [
            ...
            if($validation->fails()) {
                ...
                if($request->temp){
                    $query = Drafter::where('code',CustomHelper::decrypt($request->temp))->first();
                    ...
                    $query->status          = '1';
                    $query->save();
                    CustomHelper::saveLog($query->getTable(),$query->id,'Update data drafter '.$query->code,'Pengguna '.session('bo_nama').' telah mengubah data drafter no '.$query->code);
                }else{
                    $query = Drafter::create([
                    ...
                    CustomHelper::saveLog($query->getTable(),$query->id,'Tambah baru data drafter '.$query->code,'Pengguna '.session('bo_nama').' telah manambahkan baru data drafter no '.$query->code);
                }
    ...
    public function show(Request $request){
        $data = Drafter::where('code',CustomHelper::decrypt($request->code))->first();
        if($data){
    ...
    public function destroy(Request $request){
        ...
        if($query){
            if($query->status == '1'){
                CustomHelper::saveLog($query->getTable(),$query->id,'Drafter '.$query->code.' telah dihapus.','Pengguna '.session('bo_nama').' telah menghapus data drafter '.$query->code);
    ...
    public function showUpload(Request $request){
        ...
        if($data){
            ...
            foreach($data->drafterDetail as $rowfile){
                $images[] = [
    ...
    public function check(Request $request){		
        $query = DrafterDetail::where('name',$request->name)->first();
        ...
        if($validation->fails()) {
            ...
            if($query){
                if($query->status == '1'){
                    ...
                    CustomHelper::saveLog($query->getTable(),$query->id,'Tambah baru data file drafter '.$query->code,'Pengguna '.session('bo_nama').' telah manambahkan baru data file drafter no '.$query->code);
                    $response = [
                        'status'		=> 200,
    ...
    public function destroyFile(Request $request){
		$data = DrafterDetail::where('code',CustomHelper::decrypt($request->id))->first();
		
		$data->deleteFile();
		
		if($data->delete()){
            CustomHelper::saveLog($data->getTable(),$data->id,'Menghapus data file drafter '.$data->code,'Pengguna '.session('bo_nama').' telah menghapus data file drafter no '.$data->code);
			return response()->json([
				'status'	=> 200,
