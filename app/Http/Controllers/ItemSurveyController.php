...
use App\Models\SurveyItemDetail;
...
class ItemSurveyController extends Controller
{
    ...
    public function datatable(Request $request){
        ...
        $total_data = SurveyItem::count();
        
        $query_data = SurveyItem::where(function($query) use ($search, $request) {
                if($search) {
        ...
        $total_filtered = SurveyItem::where(function($query) use ($search, $request) {
                if($search) {
                    $query->where(function($query) use ($search, $request) {
                        $query->where('code', 'like', "%$search%")
                            ->orWhere('note', 'like', "%$search%")
    ...
    public function create(Request $request){
        ...
        try {
            ...
            if($validation->fails()) {
                ...
                if($request->temp){
                    $query = SurveyItem::where('code',CustomHelper::decrypt($request->temp))->first();
                    ...
                    $query->status              = '3';
                    $query->save();
                    CustomHelper::saveLog($query->getTable(),$query->id,'Update data hasil survei item '.$query->code,'Pengguna '.session('bo_nama').' telah mengubah data hasil survei item no '.$query->code);
                }else{
                    $query = SurveyItem::create([
                    ...
                    CustomHelper::saveLog($query->getTable(),$query->id,'Tambah baru data hasil survei item '.$query->code,'Pengguna '.session('bo_nama').' telah manambahkan baru data hasil survei item no '.$query->code);
                }
